...
def run_iterative_fix_py(max_iters: int = 5, repo_dir: str = None):
    ...
    for iteration in range(1, max_iters + 1):
        ...
        issues_before = count_static_issues(REPORT_PY)
        ...
        try:
            if repo_dir:
                ...
                # Recount issues after automatic rule-based fixes
                issues_before = count_static_issues(REPORT_PY)
                print(f"[*] Static issues found (after rule-based fixes): {issues_before}")
                if issues_before == 0:
                    print("[+] Rule-based fixes cleared all static issues.")
                    return reports
        except Exception as e:
        ...
        # 4) Re-run static analyzer now that patches have been applied and measure improvement
        # Invoke analyzer again against the same repo_dir so we can compute the new issue count.
        analyzer_cmd = "py -3 -u analyzer_py.py"
        if repo_dir:
            analyzer_cmd += f' --repo-dir "{repo_dir}"'
        subprocess.run(analyzer_cmd, shell=True, check=False, cwd=BASE_DIR)

        issues_after = count_static_issues(REPORT_PY)
        print(f"[*] Static issues found (after patch): {issues_after}")

        # Attach post-apply static count to the report entry for visibility
        report_entry["static_issues_after"] = issues_after
        # Backwards-compatible field used by the UI/table: set to post-apply count
        report_entry["static_issues"] = issues_after
        ...
        # Decide whether to abort: only abort when static issues did not decrease
        # AND dynamic tests did not improve. This allows continuing when runtime tests
        # improve even if linter counts remain the same.
        static_not_decreased = issues_after >= prev_issues
        ...
        # New abort policy:
        # - Continue iterating up to max_iters even if static issues don't decrease,
        #   as long as new patches were produced or dynamic tests improved.
        # - Abort only when no new patches were produced AND dynamic tests did not improve
        #   (this indicates we are unlikely to make further progress).
        if patches_produced == 0 and static_not_decreased and dynamic_not_improved:
            # The previous behavior aborted here. The user asked to allow the loop to
            # continue up to max_iters even if static counts do not decrease. So we
            # do not return early — only log the situation and continue.
